...
const Card = ({ video }) => {
  ...
  const thumbnail = video?.thumbnail[video?.thumbnail.length - 1];
  return (
    <Link to={`/watch?v=${video.videoId}`} className="group block">
      ...
      <div className="relative rounded-xl overflow-hidden mb-3 w-full aspect-video">
        ...
        {video.isLive && <span className="absolute bottom-1 right-1 text-xs px-2 py-0.5 rounded bg-red-600 text-white">CANLI</span>}
      </div>
