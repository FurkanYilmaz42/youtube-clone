...
const Sidebar = () => {
 ...
 const {isCollapsed} = useSidebar();
  ...
  if(isCollapsed){
    return <aside className="w-20 h-[calc(100vh-56px)] overflow-y-auto sticky top-14 max-sm:hidden">
      <div className="p-3">
      {collapsedNavItems.map((item,key)=> <Link to={item.path} key={key} className="flex flex-col items-center justify-center py-4 px-2 mx-2 rounded-lg transition hover:bg-grey ">
